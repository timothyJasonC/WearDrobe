...
export default function EditableInput({ inputType, data, classNameHead, head, defValue, handleFunc, children, hoverText }: { data: IAdmin, classNameHead?: string, head?: string, defValue:any, inputType: string, handleFunc: any, children?: React.ReactNode, hoverText: string }) {
        ...
        <>
            { head && <span className={`text-black font-semibold ${ classNameHead }`}>{ head }: </span> }
            {
                ...
                <div className="flex items-center gap-4 grow">
                    <Input className="max-w-40" ref={inputRef} type={inputType} defaultValue={defValue == data.dob ? new Date(formattedDate) : defValue} />
