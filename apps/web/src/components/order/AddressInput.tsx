    ...
    return (
        <section className={`flex flex-col gap-2 lg:gap-4 ${ forWarehouse ? 'w-full' : '' }`}>
            ...
            <div className="flex items-center justify-between max-sm:flex-col max-sm:items-start max-sm:gap-1">
                ...
                <Input className={`max-w-full ${ forWarehouse ? 'sm:max-w-96 md:max-w-[35rem] lg:max-w-96' : 'sm:max-w-80' } duration-200`} value={address} onChange={(e) => setAddress(e.target.value)} placeholder="@example: Jalan Perintis Kemerdekaan, Padang, Indonesia" disabled={!selectedCity} />
            </div>
        </section>
    );
}
